...
for imgPath in labelDict:
    ...
    text = labelDict[imgPath][2]
    # doc = nlp(text)
    # tokenizedText = np.array([x.vector for x in doc])
    imgId = imgPath
    imgId = int(imgId)
    img = Image.open(fPath)
    w, h = img.size
    if w + h > windowSize * 2.5 :#scale down large images
        print('scaling down:')
        print(imgId)
        scaleFactor = int((w + h) / (windowSize * 2.5))
        print(scaleFactor, w, h)
    ...
    if img.size[0] > windowSize * 2 or img.size[1] > windowSize *  2:#divide into sections and crop
        ...
        for w in range(widthMult):
            for h in range(heightMult):
                ...
                print(np.asarray(croppedImg).shape)
                if np.asarray(croppedImg).shape[0] == 0:
                    quit()
                imgDataToDump.append(np.asarray(croppedImg).tolist())
                labelDataToDump.append(label)
                textToDump.append(text)
